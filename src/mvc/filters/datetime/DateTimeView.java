...
public class DateTimeView 
{
	...
	private void draw()
	{
		...
		model.startDate.disablePopup();
		model.stopDate.disablePopup();
		
		
		x = 0;
		y = 0;
		
		model.startPanel.add(model.startDate.getPanel(), grid.set(x, y));
		model.startPanel.add(model.startTime.getPanel(), grid.set(++x, y));
		
		
		x = 0;
		y = 0;
		
		model.stopPanel.add(model.stopDate.getPanel(), grid.set(x,   y));
		model.stopPanel.add(model.stopTime.getPanel(), grid.set(++x, y));
