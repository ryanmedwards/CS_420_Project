...
public class CollectView
{
	...
	protected void draw()
	{
		...
		model.filtersPanel.add(model.timeLabel, grid.set(--x, ++y));
		model.filtersPanel.add(model.timeBox, grid.set(++x,   y));
		
		x = 0;
		
		model.filtersPanel.add(model.durationLabel, grid.set(x, ++y));
		
		grid.setAnchor(Grid.EAST);
		model.filtersPanel.add(model.durationSpinner, grid.set(++x, y));
